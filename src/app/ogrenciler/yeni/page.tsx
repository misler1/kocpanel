...
// YKS yılı: Ağustos itibariyle otomatik
function getYksYear() {
  const now = new Date();
...
export default function YeniOgrenciPage() {
  ...
  const [avatarColor, setAvatarColor] = useState('av-blue');
  ...
  // Veli bilgileri
  const [guardianSource, setGuardianSource] = useState<'anne' | 'baba' | 'diger'>('diger');
  const [guardianName, setGuardianName] = useState('');
  const [guardianPhone, setGuardianPhone] = useState('');
  const [guardianRelation, setGuardianRelation] = useState('');

  // YKS sonuçları
  const [yksYear, setYksYear] = useState(getYksYear());
  const [tytScore, setTytScore] = useState('');
  const [sayScore, setSayScore] = useState('');
  const [eaScore, setEaScore] = useState('');
  const [sozScore, setSozScore] = useState('');
  const [tytRank, setTytRank] = useState('');
  const [sayRank, setSayRank] = useState('');
  const [eaRank, setEaRank] = useState('');
  const [sozRank, setSozRank] = useState('');
  ...
  return (
    <div className="mx-auto max-w-2xl pb-12">
      <div className="mb-5 flex items-center gap-3">
        ...
        <h1 className="text-[18px] font-medium text-gray-900">Yeni öğrenci ekle</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border border-gray-200 bg-white px-5 py-5 space-y-4">
          ...
          <div>
            <p className="mb-2 text-sm text-gray-500">Veli bilgilerini nereden alalım?</p>
            <div className="flex gap-3">
              {[
                { value: 'anne', label: 'Anne bilgilerinden al' },
                { value: 'baba', label: 'Baba bilgilerinden al' },
                { value: 'diger', label: 'Ayrıca gir' },
              ].map((opt) => (
                <label key={opt.value} className="flex cursor-pointer items-center gap-2">
                  <input type="radio" name="guardian_source"
                    checked={guardianSource === opt.value}
                    onChange={() => handleGuardianSource(opt.value as 'anne' | 'baba' | 'diger')}

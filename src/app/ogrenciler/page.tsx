...
const AVATAR_COLOR_LIST = Object.keys(AVATAR_COLORS);
...
export default async function OgrencilerPage() {
  ...
  const active = all.filter((s) => s.status !== 'pasif');
  const passive = all.filter((s) => s.status === 'pasif');
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {/* Arama (client-side için statik görüntü) */}
      <div className="mb-4 flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-400">
        <IconSearch size={16} />

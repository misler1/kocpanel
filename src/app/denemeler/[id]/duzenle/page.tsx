...
function DuzenleForm() {
  ...
  // Mevcut deneme verilerini ve öğrencileri yükle
  useEffect(() => {
    async function load() {
      ...
      setAnalysisDone(exam.analysis_done ?? false);
      setAnalysisNotes(exam.analysis_notes ?? '');
      setLinkedTytId(exam.linked_exam_id ?? '');

      // Puan alanları
      if (exam.tyt_puan != null) setTytPuan(String(exam.tyt_puan));
      if (exam.say_puan != null) setSayPuan(String(exam.say_puan));
      if (exam.ea_puan != null) setEaPuan(String(exam.ea_puan));
      if (exam.soz_puan != null) setSozPuan(String(exam.soz_puan));
      if (exam.lgs_puan != null) setLgsPuan(String(exam.lgs_puan));

      // Ders sonuçlarını doldur
      const sr = exam.subject_results ?? {};
      if (exam.exam_type === 'TYT') {
        setTytResults((prev) => ({ ...initResults(TYT_SUBJECTS), ...sr }));
      } else if (exam.exam_type === 'AYT') {
        setAytResults((prev) => ({ ...initResults(AYT_SUBJECTS), ...sr }));
      } else if (exam.exam_type === 'LGS') {
        setLgsResults((prev) => ({ ...initResults(LGS_SUBJECTS), ...sr }));
      }
  ...
  // AYT seçilince mevcut TYT denemelerini yükle
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error: err } = await (supabase as any).from('exams').update({
  ...
  if (pageLoading) {
    return <div className="p-8 text-sm text-gray-400">Yükleniyor...</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-2xl pb-16">
      <div className="mb-5 flex items-center gap-3">
        ...
        <h1 className="text-[18px] font-medium text-gray-900">Deneme sonucu düzenle</h1>
      </div>
...
export default function DuzenleDenemePage() {
  return (
